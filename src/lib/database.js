...
export function translateSqlError(error, t) {
  ...
  if (!t) {
    ...
    if (lower.includes('syntax error')) return 'SQL 문법에 오류가 있어요. SELECT, FROM, WHERE 같은 키워드 순서와 쉼표를 확인해보세요.';
    if (lower.includes('ambiguous column')) return '어느 테이블의 컬럼인지 모호해요. e.name처럼 테이블 별칭을 붙여보세요.';
    if (lower.includes('misuse of aggregate') || lower.includes('aggregate')) return '집계 함수 사용 위치를 확인해보세요. 집계 조건은 WHERE가 아니라 HAVING에 쓰는 경우가 많습니다.';
    if (lower.includes('incomplete input')) return 'SQL 문장이 끝나지 않았어요. 괄호, 따옴표, 조건식을 마저 닫아주세요.';
    return `SQL 실행 중 오류가 발생했어요. ${message}`;
  }

  if (lower.includes('no such column')) {
    const column = message.match(/no such column: ([^\s]+)/i)?.[1];
    return column ? t('db.error.noColumn.named', { name: column }) : t('db.error.noColumn');
  }
  if (lower.includes('no such table')) {
    const table = message.match(/no such table: ([^\s]+)/i)?.[1];
    return table ? t('db.error.noTable.named', { name: table }) : t('db.error.noTable');
  }
  if (lower.includes('syntax error')) return t('db.error.syntax');
  if (lower.includes('ambiguous column')) return t('db.error.ambiguous');
  if (lower.includes('misuse of aggregate') || lower.includes('aggregate')) return t('db.error.aggregate');
  if (lower.includes('incomplete input')) return t('db.error.incomplete');
  return t('db.error.generic', { message });
}
...
export function gradeQuery(userSql, answerSql, checkOrder = false, options = {}, t) {
  const tr = (key, vars) => t ? t(key, vars) : key;
  try {
    const userRes = db.exec(userSql.trim());
    const ansRes = db.exec(answerSql.trim());

    if (!userRes.length && !ansRes.length) return { correct: true, checks: [tr('grade.empty.check')] };
    if (!userRes.length) return { correct: false, msg: tr('grade.empty.msg'), checks: [tr('grade.empty.emptyCheck')] };
    if (!ansRes.length) return { correct: false, msg: tr('grade.unexpected') };
    ...
    if (userRows.length !== ansRows.length) {
      return {
        correct: false,
        msg: tr('grade.rowCount.msg', { ans: ansRows.length, user: userRows.length }),
        checks: [tr('grade.rowCount.check1'), tr('grade.rowCount.check2')],
      };
    }
    ...
    if (userColumns.length !== ansColumns.length) {
      return {
        correct: false,
        msg: tr('grade.colCount.msg', { ans: ansColumns.length, user: userColumns.length }),
        checks: [tr('grade.colCount.check')],
      };
    }
    ...
    if (options.strictColumns && JSON.stringify(userColumns) !== JSON.stringify(ansColumns)) {
      return {
        correct: false,
        msg: tr('grade.colName.msg'),
        checks: [
          tr('grade.colName.check1', { cols: ansColumns.join(', ') }),
          tr('grade.colName.check2', { cols: userColumns.join(', ') }),
        ],
      };
    }
    ...
    if (JSON.stringify(userSerial) !== JSON.stringify(ansSerial)) {
      ...
      if (checkOrder && JSON.stringify(unorderedUser) === JSON.stringify(unorderedAnswer)) {
        return {
          correct: false,
          msg: tr('grade.order.msg'),
          checks: [tr('grade.order.check')],
        };
      }
      ...
      return {
        correct: false,
        msg: diff
          ? tr('grade.value.msg', { row: diff.row, col: diff.col, expected: diff.expected, actual: diff.actual })
          : tr('grade.value.alt'),
        checks: [tr('grade.value.check1'), tr('grade.value.check2')],
      };
    }

    if (JSON.stringify(userColumns) !== JSON.stringify(ansColumns)) {
      checks.push(tr('grade.ok.colAlias'));
    } else {
      checks.push(tr('grade.ok.col'));
    }
    if (checkOrder) checks.push(tr('grade.ok.order'));

    return { correct: true, checks };
  } catch (e) {
    return { correct: false, msg: translateSqlError(e, t), checks: [tr('grade.execFail.check')] };
  }
}
